...
export default function AdminMessagesPage() {
  // صفحة Client لكن نضع useSearchParams داخل Suspense حسب متطلبات Next
  return (
    ...
    <div className="flex flex-col h-screen p-4">
      ...
      <div className="mx-auto w-full max-w-xl flex flex-col border-2 border-[#ffd15c] bg-black text-[#ffd15c] rounded h-96 mt-32">
        ...
        <form onSubmit={handleSend} className="flex items-center gap-2 p-2 border-t border-[#ffd15c] bg-black">
          ...
          <button type="button" onClick={() => fileInp.current?.click()} className="p-1">
            📎
          </button>
          <button type="button" onClick={() => photoInp.current?.click()} className="p-1">
            🖼️
          </button>
